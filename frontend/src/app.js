...
const el = {
  ...
  helpSidebarClose: $("helpSidebarClose"),
  helpSidebarContent: $("helpSidebarContent"),
  helpModeToggle: $("helpModeToggle"),
  parlorModeToggle: $("parlorModeToggle"),
...
const runJob = async () => {
  ...
  const prompt =
    payload.text || "Standard-Qualitätsprüfung (kein spezifischer Textprompt)";
  addChatMessage("user", prompt);
  addChatMessage(
    "system",
    "Führe physikalische Analyse und KI-Bewertung durch...",
  );

  try {
    const wsResp = await ws.request("inspect", payload);
    const data = wsResp.payload || wsResp;
    setJsonView(el.jobResult, data);

    const explanation =
      data.report?.explanation ||
      data.explanation ||
      "Inspektion abgeschlossen. Keine generative Text-Erklärung im Resultat.";
    addChatMessage("bot", explanation);
  } catch {
    try {
      ...
      const explanation =
        restResp.report?.explanation ||
        restResp.explanation ||
        "Inspektion abgeschlossen. Keine generative Text-Erklärung im Resultat.";
      addChatMessage("bot", explanation);
    } catch (err) {
      ...
      addChatMessage(
        "system",
        `Systemfehler bei der Analyse: ${err.message || err}`,
      );
    }
  }
...
/* --- Roboto Labdroid: Help Mode (Phase 2) & Parlor Mode (Phase 3) --- */

const renderHelpContent = (key) => {
  const content = helpDict[key] || helpDict.default;
  if (!el.helpSidebarContent) return;

  el.helpSidebarContent.innerHTML = `
    <div class="help-card">
      <div class="help-card-title">${content.title}</div>
      <p class="help-card-text">${content.desc}</p>
      ${content.tip ? `<div class="help-card-tip"><strong>Praxistipp:</strong> ${content.tip}</div>` : ""}
      ${content.compliance ? `<div class="help-card-compliance"><strong>Sicherheits-/Qualitätsrichtlinie:</strong> ${content.compliance}</div>` : ""}
    </div>
  `;
};

const handleHelpClose = () => {
  if (el.helpModeToggle) {
    el.helpModeToggle.checked = false;
    el.helpModeToggle.dispatchEvent(new Event("change"));
  }
};

const initHelpMode = () => {
  if (!el.helpModeToggle) return;

  renderHelpContent("default");

  el.helpModeToggle.addEventListener("change", () => {
    const active = el.helpModeToggle.checked;
  ...
  el.helpSidebarClose?.addEventListener("click", handleHelpClose);

  Object.keys(helpDict).forEach((id) => {
    if (
      id === "default" ||
      id === "helpModeToggle" ||
      id === "parlorModeToggle"
    ...
    targetElement.classList.add("help-trackable");

    const label = targetElement.closest("label");
    if (label) {
      label.classList.add("help-trackable");

      if (!label.querySelector(".help-indicator-badge")) {
        const badge = document.createElement("span");
        badge.className = "help-indicator-badge";
        badge.textContent = "?";
        badge.title = "Mauszeiger für Details bewegen";
    ...
    const onEnter = () => {
      if (!el.helpModeToggle.checked) return;
      if (debounceTimer) {
...
/* --- Parlor Mode: Voice Activity Detection (VAD) & Text-To-Speech (TTS) --- */

const speakText = (text) => {
  if (!("speechSynthesis" in window)) return;
  if (!el.parlorModeToggle?.checked) return;
...
const addChatMessage = (sender, text) => {
  ...
  const infoPlaceholder = el.chatHistory.querySelector(
  ...
  let senderName = "";
  if (sender === "user") senderName = `👨‍🔬 Du (Laborprüfer) [${timeStr}]:`;
  else if (sender === "bot")
    senderName = `🤖 KI (Roboto Labdroid) [${timeStr}]:`;
  else senderName = `⚙️ System [${timeStr}]:`;
  ...
  headerSpan.style.marginBottom = "4px";
...
const exportJsonResult = () => {
  if (!el.jobResult) return;
  const jsonText = el.jobResult.value.trim();
  if (!jsonText) {
    alert("Kein Job-Ergebnis zum Exportieren vorhanden.");
    return;
  }
  ...
  try {
    ...
    alert(
      "Das JSON im Textfeld enthält Syntaxfehler und kann nicht valide exportiert werden:\n" +
        err.message,
    );
    return;
  }
  ...
  a.href = url;
  a.download = `labdroid_analysis_export_${Date.now()}.json`;
  ...
  if (el.exportStatus) {
    el.exportStatus.style.display = "inline";
    setTimeout(() => {
      el.exportStatus.style.display = "none";
    }, 2000);
  }
};

let parlorInterval = null;
let silenceTimer = null;

const checkParlorVad = () => {
  if (!state.audioActive) return;

  const rms = Number(el.audioLevel?.value) || 0;
  const threshDb = Number(el.threshold?.value) || -24;
  const threshLinear = Math.pow(10, threshDb / 20);

  if (rms > threshLinear) {
    if (!state.isSpeaking) {
      state.isSpeaking = true;
      console.log("[Parlor Mode] Sprachaktivität erkannt...");
      if (silenceTimer) {
    ...
    if (state.isSpeaking) {
      if (!silenceTimer) {
        console.log(
          "[Parlor Mode] Rauschen unter Schwellenwert -> Stille erkannt, wage Trigger...",
        );
        silenceTimer = setTimeout(async () => {
          console.log(
            "[Parlor Mode] Stille erreicht! Automatische Inspektion wird gestartet...",
          );
          ...
          const promptText =
            el.text.value.trim() ||
            "🎙️ Hands-Free Sprach-Trigger (Allgemeine Inspektion)";
          addChatMessage("user", promptText);
          addChatMessage(
            "system",
            "Sprach-Aktivierung erkannt. Analysiere Livedaten...",
          );

          try {
            setJsonView(el.jobResult, {
              info: "[Parlor] Sprachsteuerung: Inspektion gestartet...",
            });
            const payload = currentJobPayload();
            const wsResp = await ws.request("inspect", payload);
            const data = wsResp.payload || wsResp;
            ...
            if (data.report?.explanation) {
              speakText(data.report.explanation);
            } else if (data.explanation) {
              speakText(data.explanation);
            }
          } catch {
            try {
              const restResp = await startJobRest(
                state.baseUrl,
                currentJobPayload(),
              );
              setJsonView(el.jobResult, restResp);
              ...
              if (restResp.report?.explanation) {
                speakText(restResp.report.explanation);
              } else if (restResp.explanation) {
                speakText(restResp.explanation);
              }
            } catch (err) {
              setJsonView(el.jobResult, { error: String(err) });
              addChatMessage(
                "system",
                `Systemfehler im Parlor-Modus: ${err.message || err}`,
              );
            }
          } finally {
...
const initParlorMode = () => {
  if (!el.parlorModeToggle) return;

  el.parlorModeToggle.addEventListener("change", () => {
    const active = el.parlorModeToggle.checked;
    console.log("[Parlor Mode] Aktivitätsstatus:", active);

    if (active) {
      if (!state.audioActive) {
        setJsonView(el.jobResult, {
          info: "[Parlor] Starte Akustikschnittstelle zur Spracherkennung...",
        });
        toggleAudio().catch((err) => {
          console.error(
            "Audio konnte im Parlor-Modus nicht gestartet werden:",
            err,
          );
        });
      }

      if (parlorInterval) clearInterval(parlorInterval);
      parlorInterval = setInterval(checkParlorVad, 100);
    } else {
      if (parlorInterval) {
        clearInterval(parlorInterval);
        parlorInterval = null;
      }
  ...
  el.clearChatBtn?.addEventListener("click", () => {
    if (el.chatHistory) {
      el.chatHistory.innerHTML = `
        <div style="font-style: italic; color: var(--muted); text-align: center; font-size: 11px; padding: 15px 0;">
          Gesprächsverlauf geleert. Starte eine neue Unterhaltung.
        </div>
      `;
    }
  });

  el.exportResultBtn?.addEventListener("click", exportJsonResult);
};
