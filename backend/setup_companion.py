...
def _check_imports() -> CheckResult:
    ...
    if missing:
        return CheckResult(
            ...
            hint="Im Ordner backend: `uv sync` ausführen.",
        )
...
def _render(results: list[CheckResult]) -> int:
    print("\n=== Roboto Labdroid Setup Companion · Status ===\n")
    for item in results:
...
def _env_template_local(args: argparse.Namespace) -> str:
    return "\n".join(
        [
            "# Roboto Labdroid Setup-Companion (local profile)",
...
def _env_template_online(args: argparse.Namespace) -> str:
    ...
    return "\n".join(
        [
            "# Roboto Labdroid Setup-Companion (online profile)",
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Roboto Labdroid Setup-Companion: Preflight-Checks und env-Profile für lokal/online.",
    )
